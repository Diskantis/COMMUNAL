...
from res.UIC_CLASS_KOMM_ED import UiWinAdd
from res.DLL_CLASS_KOMM_ED import dt_month, dt_year, month, convert_month, selected_period, \
                                Period, Save_OR, SQLite3_Data_Base
from COUNTERS_UIC_ED import UiWinPokazanya
...
class PokazSchet(QtWidgets.QWidget, UiWinPokazanya):
    def __init__(self):
        ...
        self.save_or_ps = Save_OR()

        # ВЫБОР ПЕРИОДА
        selected_period(self.comboBox_month_PS, self.comboBox_year_PS)
        ...
        self.label_month_year_PS.setText(convert_month(dt_month) + " " + dt_year)  # заголовок ("Месяц Год")
        self.comboBox_month_PS.setCurrentIndex(self.current_month_index)  # устанавливает текущий месяц ("Месяц")
        self.comboBox_year_PS.setCurrentText(dt_year)  # устанавливает текущий год ("Год")
        ...
        self.win_pole = [self.lineEdit_pred_P, self.lineEdit_post_P,
                         ...
                         self.label_error_PS, self.label_GL_V_1_PS, self.label_GL_V_2_PS]
        ...
        self.btn_Save_PS.clicked.connect(self.btn_save_ps)
        self.btn_Cancel_PS.clicked.connect(self.btn_cancel_ps)
    ...
    def start_period(self):
        if self.label_month_year_PS.text() == 'Декабрь 2006' and self.comboBox_month_PS.count() == 12 \
                or int(self.comboBox_year_PS.currentText()) == 2006 and self.comboBox_month_PS.currentIndex() <= 11 \
                and self.comboBox_month_PS.count() == 12:
            self.comboBox_month_PS.clear()
            self.comboBox_month_PS.addItems(month[5:12])
        elif self.label_month_year_PS.text() == 'Декабрь 2006' and self.comboBox_month_PS.count() == 7 \
                or int(self.comboBox_year_PS.currentText()) >= 2007 and self.comboBox_month_PS.currentIndex() >= 0 \
                and self.comboBox_month_PS.count() == 7:
            self.comboBox_month_PS.clear()
            self.comboBox_month_PS.addItems(month)

    def label_period(self):
        self.start_period()
        self.current_month_index = self.period_ps.label_sel_period()
        self.read_pokaz_schet()

    def btn_period_left(self):
        self.start_period()
        if self.label_month_year_PS.text() == "Июнь 2006":
            self.btn_Left_PS.setEnabled(False)
        else:
    ...
    def btn_period_right(self):
        self.start_period()
        if self.label_month_year_PS.text() != "Май 2006":
            self.btn_Left_PS.setEnabled(True)
    ...
    def keyPressEvent(self, event):  # отключает режим редактирования по нажатию клавиши Esc
        if event.key() == Qt.Key_Escape:
            if month.index(self.label_month_year_PS.text().split()[0]) + 1 in \
                    SQLite3_Data_Base.sqlite3_read_data(
                        self.data_base, 'Pokazanya_year_' + str(self.comboBox_year_PS.currentText()), 'id')[0]:
    ...
    def read_pokaz_schet(self):
        ...
        if month.index(self.label_month_year_PS.text().split()[0]) + 1 in \
            ...
            self.label_GL_V_1_PS.setPixmap(QtGui.QPixmap("res/img/Galochka_1.png"))
            self.label_GL_V_2_PS.setPixmap(QtGui.QPixmap("res/img/Galochka_1.png"))
        ...
        if not SQLite3_Data_Base.sqlite3_read_data(self.data_base, table)[0] and \
                month[self.comboBox_month_PS.currentIndex()] == "Январь":
        ...
        for i in range(len(read_table)):
            ...
            if self.label_month_year_PS.text() == pred_pokaz[1]:
                ...
                pred_month = month[self.comboBox_month_PS.currentIndex() - 1]
        ...
        try:
            ...
            self.label_error_PS.show()
            self.label_error_PS.setText('Нет значений для этого периода')
    ...
    def summa_month_ras(self, n):
        try:
            ...
            self.label_error_PS.show()
            self.label_error_PS.setText('Должно быть значение!')

    # создает список значений полей для записи в таблицу
    def create_list_pokaz_schet(self):  # список показаний за месяц
        data = [self.comboBox_month_PS.currentIndex() + 1,
                self.comboBox_month_PS.currentText() + " " + self.comboBox_year_PS.currentText()]
        try:
